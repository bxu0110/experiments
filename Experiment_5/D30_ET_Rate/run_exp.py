...
from env_resdisnew60_cnvRate5 import Env_Resdisnew60_cnvRate5
import numpy as np
...
policies.append([2, 2, 2, 2, 2, 2, 2, 2, 2, 2])  # intermediate search
policies.append([4, 4, 4, 4, 4, 4, 4, 4, 4, 4])  # global
policies.append([4, 4, 3, 3, 2, 2, 1, 1, 0, 0])  # uniform converge
policies.append([4, 0, 4, 0, 4, 0, 4, 0, 4, 0])  # policy_expl_expl
policies.append([4, 2, 0, 4, 2, 0, 4, 2, 0, 4])  # 3-step expl-expl
policies.append(np.random.randint(0, 4, size=10))  # policy_random
...
for func_num in range(16, 21):
    all_returns = []
    all_fitness = []
    experiment = "ET_cnvRange"
    results_file_reward = f"{experiment}_rewards(f{func_num}).csv"
    results_file_fitness = f"{experiment}_fitness(f{func_num}).csv"

    for policy in policies:
        environment = Env_Resdisnew60_cnvRate5(func_num, dim=dim, minimum=fDeltas[func_num - 1])
